...
- `config_image_hook__rock-5b-plus(rootfs, overlay, suite)` — runs **inside `config-image.sh`** after the kernel/u-boot are installed, with the chroot mounted. It `chroot ... apt-get install`s the board's multimedia firmware (panfork Mesa, libmali, camera-engine) plus extra runtime packages (nodejs, GStreamer incl. `gstreamer1.0-rockchip1`, NetworkManager/ModemManager, hostapd, nftables, v4l-utils, nginx, sqlite3, alsa-utils), copies files from `overlay/`, and `systemctl enable`s board services (e.g. `alsa-audio-config`). This is the place to add board-specific runtime setup.
- `build_image_hook__rock-5b-plus(writable)` — runs in `build-image.sh` after the rootfs is laid onto the image and right *before* `u-boot-update`, with `$1` = the mounted writable root. Used here to apply device-tree overlays: for each `.dts` in `scripts/overlays/`, `scripts/install-dt-overlay.sh` compiles it to a `.dtbo`, drops it in the kernel's `device-tree/rockchip/overlay/` dir, and appends it to `U_BOOT_FDT_OVERLAYS` in `/etc/default/u-boot` so `u-boot-update` writes it into the extlinux config. Current overlays: `hdmiin-audio` (HDMI-in audio capture fix, issue #1057) and `fan-cooling` (pin the pwm-fan speed). It also appends `net.ifnames=0 biosdevname=0` to `U_BOOT_PARAMETERS` in `/etc/default/u-boot` (classic `eth0`/`wlan0` interface names). This hook is the right place for anything that must touch the final on-disk image (dtbs, bootloader config) rather than the chroot.
