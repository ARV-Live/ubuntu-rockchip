...
- `build_image_hook__rock-5b-plus(writable)` — runs in `build-image.sh` after the rootfs is laid onto the image and right *before* `u-boot-update`, with `$1` = the mounted writable root. Used here to apply device-tree overlays: for each `.dts` in `scripts/overlays/`, `scripts/install-dt-overlay.sh` compiles it to a `.dtbo`, drops it in the kernel's `device-tree/rockchip/overlay/` dir, and appends it to `U_BOOT_FDT_OVERLAYS` in `/etc/default/u-boot` so `u-boot-update` writes it into the extlinux config. Current overlays: `hdmiin-audio` (HDMI-in audio capture fix, issue #1057) and `fan-cooling` (pin the pwm-fan speed). This hook is the right place for anything that must touch the final on-disk image (dtbs, bootloader config) rather than the chroot.
